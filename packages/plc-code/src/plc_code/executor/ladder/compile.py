...
Field and metadata source is generated by reusing the very same
``SCLTranspiler`` helpers (``_generate_variables`` / ``_generate_metadata``) the
SCL path uses, so defaults and metadata stay byte-for-byte identical. Only the
``execute()`` body diverges: instead of inlined SCL it delegates to a
:class:`LadderInterpreter` built once per class from the flattened ladder
program.
"""
...
def _generate_fields_source(block: Block) -> str:
    ...
    Reuses :class:`SCLTranspiler`'s ``_generate_variables`` and
    ``_generate_metadata`` so the generated fields and metadata tuples are
    identical to those the SCL path produces for the same VAR sections.
    """
    ...
    # Reset the emit buffer and indent one level (into the class body).
    transpiler._lines = []
    transpiler._ctx = CodeGenContext().push()
    transpiler._generate_variables()
    transpiler._generate_metadata()
    return "\n".join(transpiler._lines)
